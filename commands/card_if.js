...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        if (bmi < 0) {
            // 如何判斷 大或小
            await interaction.reply(`史考特比利奧拉大${bmi*-1}`); // 如果符合，應該回覆甚麼訊息
        } else if (bmi > 0) {
            // 應該要如何判斷剩下的
            await interaction.reply(`利奧拉比史考特大${bmi}`); // 如果符合，應該回覆甚麼訊息
        } else {
            // 最後一個應該是甚麼情況
            await interaction.reply(`史考特與利奧拉一樣大`); // 如果符合，應該回覆甚麼訊息
        }
    },
};
